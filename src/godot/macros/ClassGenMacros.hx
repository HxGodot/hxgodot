	...
	static function _generateClasses(_api:Dynamic) {
		...
		for (c in classes) {
			...
			if (c.is_instantiable != null && c.is_instantiable) {
				var cname = c.name;
				var tmp = c.is_refcounted ? macro class {
					public function new() {
						super();
					};
				} : macro class {
				...
				for (m in cast(c.methods, Array<Dynamic>)) {
					...
					if (m.is_virtual) {
						...
						binds.push({
							...
							macros: {
								field: (macro class {
									@:noCompletion public static var $mname:godot.Types.GDExtensionPtrBuiltInMethod;
								}).fields[0],
								fieldSetter: [
				...
				if (bind.type != FunctionBindType.VIRTUAL_METHOD) {
					if (bind.macros.fieldSetter != null) {
						for (s in bind.macros.fieldSetter)
							pointerInits.push(Context.parse(s, Context.currentPos()));
					}
		...
		for (b in builtins) {
			...
			for (c in cast(b.constructors, Array<Dynamic>)) {
				...
				binds.push({
					...
					macros: {
						field: (macro class {
							@:noCompletion static var $cname:godot.Types.GDExtensionPtrConstructor;
						}).fields[0],
						fieldSetter: [
			...
			if (b.has_destructor) {
				binds.push({
					...
					macros: {
						field: (macro class {
							@:noCompletion static var _destructor:godot.Types.GDExtensionPtrDestructor;
						}).fields[0],
						fieldSetter: [
			...
			if (b.members != null) {
				for (m in cast(b.members, Array<Dynamic>)) {
					...
					binds.push({
						...
						macros: {
							field: (macro class {
								@:noCompletion static var $mname:godot.Types.GDExtensionPtrGetter;
							}).fields[0],
							fieldSetter: [
								'var name_${m.name}:godot.variant.StringName = "${m.name}"',
					...
					binds.push({
						...
						macros: {
							field: (macro class {
								@:noCompletion static var $mname:godot.Types.GDExtensionPtrSetter;
							}).fields[0],
							fieldSetter: [
								'var name_${m.name}:godot.variant.StringName = "${m.name}"',
			...
			for (m in cast(b.methods, Array<Dynamic>)) {
				...
				binds.push({
					...
					hasVarArg: hasVarArg,
					macros: {
						field: hasVarArg ? null : (macro class {
							@:noCompletion static var $mname:godot.Types.GDExtensionPtrBuiltInMethod;
						}).fields[0],
			...
			if (b.operators != null) {
				for (o in cast(b.operators, Array<Dynamic>)) {
					...
					binds.push({
						...
						macros: {
							field: (macro class {
								@:noCompletion static var $oname:godot.Types.GDExtensionPtrOperatorEvaluator;
							}).fields[0],
							fieldSetter: [
			...
			if (b.indexing_return_type != null) {
				...
				if (TypeMacros.isTypeAllowed(b.indexing_return_type)) {
					...
					binds.push({
						...
						macros: {
							field: (macro class {
								@:noCompletion static var $iname:godot.Types.GDExtensionPtrIndexedGetter;
							}).fields[0],
							fieldSetter: [
					...
					binds.push({
						...
						macros: {
							field: (macro class {
								@:noCompletion static var $iname:godot.Types.GDExtensionPtrIndexedSetter;
							}).fields[0],
							fieldSetter: [
					...
					for (s in bind.macros.fieldSetter)
						switch (bind.type) {
							case FunctionBindType.CONSTRUCTOR(index):
								pointerConstructors.push(Context.parse(s, Context.currentPos()));
							case FunctionBindType.DESTRUCTOR, FunctionBindType.OPERATOR:
								pointerConstructors.push(Context.parse(s, Context.currentPos()));
							default:
								pointerInits.push(Context.parse(s, Context.currentPos()));
						}
			...
			cls.meta = [
				...
				},
				{
					name: ":headerCode",
					params: [
						macro "#include <godot_cpp/gdextension_interface.h>\n#include <godot_cpp/core/defs.hpp>\n#include <array>"
					],
					pos: Context.currentPos()
				}
			];
